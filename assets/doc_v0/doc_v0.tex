...
\item[] \keyword{Locally Assembled}: Every local stiffness matrices are assembled and store into $N_e$ \keyword{FullMatrix} data structures. Compared to  the \keyword{Assembled} mode, this enable application of $\mathbb{K}_{\param{\alpha}}$ in parallel, but increases the memory footprint,
...
\end{itemize}
For both modes, the available operation is \keyword{MltAdd} exclusively since, by construction, the stiffness operator has a non-empty kernel, thus is not invertible.
