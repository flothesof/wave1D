...
def test_assembled_mass_p1():
    ...
    mass = mass_assembler.assemble_mass(lambda s: cte_density, fe_space)
...
def test_lumped_mass_p2():
    ...
    assemb_mass = mass_assembler.assemble_mass(density, fe_space, fe_op.AssemblyType.ASSEMBLED).data.todense()
    lumped_mass = np.diag(mass_assembler.assemble_mass(density, fe_space, fe_op.AssemblyType.LUMPED).data)
